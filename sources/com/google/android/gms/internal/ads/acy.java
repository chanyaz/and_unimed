package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class acy extends acx {
    private static final Object s = new Object();
    private static final String t = acy.class.getSimpleName();
    @VisibleForTesting
    private static boolean u = false;
    private static long v = 0;
    protected boolean r = false;
    private String w;
    private boolean x = false;
    private boolean y = false;
    @VisibleForTesting
    private ady z;

    protected acy(Context context, String str, boolean z) {
        super(context);
        this.w = str;
        this.r = z;
    }

    private static adv a(adn adn, MotionEvent motionEvent, DisplayMetrics displayMetrics) {
        Throwable e;
        Method a = adn.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new adv((String) a.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException e2) {
            e = e2;
            throw new zzcw(e);
        } catch (InvocationTargetException e3) {
            e = e3;
            throw new zzcw(e);
        }
    }

    protected static synchronized void a(Context context, boolean z) {
        synchronized (acy.class) {
            if (!u) {
                v = System.currentTimeMillis() / 1000;
                a = b(context, z);
                u = true;
            }
        }
    }

    private static void a(List<Callable<Void>> list) {
        if (a != null) {
            ExecutorService c = a.c();
            if (c != null && !list.isEmpty()) {
                try {
                    c.invokeAll(list, ((Long) akc.f().a(amn.bD)).longValue(), TimeUnit.MILLISECONDS);
                } catch (Throwable e) {
                    Log.d(t, String.format("class methods got exception: %s", new Object[]{adw.a(e)}));
                }
            }
        }
    }

    private static adn b(Context context, boolean z) {
        if (a == null) {
            synchronized (s) {
                if (a == null) {
                    adn a = adn.a(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (a.b()) {
                        if (((Boolean) akc.f().a(amn.bJ)).booleanValue()) {
                            a.a("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        a.a("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        a.a("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        a.a("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        a.a("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        a.a("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        a.a("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        a.a("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        a.a("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        a.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        a.a("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        a.a("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        a.a("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        a.a("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        a.a("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        a.a("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        a.a("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        a.a("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        a.a("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        a.a("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        a.a("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    a = a;
                }
            }
        }
        return a;
    }

    protected final long a(StackTraceElement[] stackTraceElementArr) {
        Throwable e;
        Method a = a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (a == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new adl((String) a.invoke(null, new Object[]{stackTraceElementArr})).a.longValue();
        } catch (IllegalAccessException e2) {
            e = e2;
        } catch (InvocationTargetException e3) {
            e = e3;
        }
        throw new zzcw(e);
    }

    protected final adv a(MotionEvent motionEvent) {
        Throwable e;
        Method a = a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new adv((String) a.invoke(null, new Object[]{motionEvent, this.q}));
        } catch (IllegalAccessException e2) {
            e = e2;
            throw new zzcw(e);
        } catch (InvocationTargetException e3) {
            e = e3;
            throw new zzcw(e);
        }
    }

    protected final wr a(Context context, View view, Activity activity) {
        List asList;
        wr wrVar = new wr();
        if (!TextUtils.isEmpty(this.w)) {
            wrVar.b = this.w;
        }
        adn b = b(context, this.r);
        if (b.b()) {
            adv a;
            try {
                a = a(b, this.b, this.q);
                wrVar.g = a.a;
                wrVar.h = a.b;
                wrVar.i = a.c;
                if (this.p) {
                    wrVar.u = a.d;
                    wrVar.v = a.e;
                }
            } catch (zzcw e) {
            }
            xq xqVar = new xq();
            if (this.d > 0 && adw.a(this.q)) {
                xqVar.m = Long.valueOf(adw.a(this.k, this.q));
                xqVar.n = Long.valueOf(adw.a((double) (this.n - this.l), this.q));
                xqVar.o = Long.valueOf(adw.a((double) (this.o - this.m), this.q));
                xqVar.r = Long.valueOf(adw.a((double) this.l, this.q));
                xqVar.s = Long.valueOf(adw.a((double) this.m, this.q));
                if (this.p && this.b != null) {
                    long a2 = adw.a((double) (((this.l - this.n) + this.b.getRawX()) - this.b.getX()), this.q);
                    if (a2 != 0) {
                        xqVar.p = Long.valueOf(a2);
                    }
                    a2 = adw.a((double) (((this.m - this.o) + this.b.getRawY()) - this.b.getY()), this.q);
                    if (a2 != 0) {
                        xqVar.q = Long.valueOf(a2);
                    }
                }
            }
            try {
                adv a3 = a(this.b);
                xqVar.a = a3.a;
                xqVar.b = a3.b;
                xqVar.h = a3.c;
                if (this.p) {
                    xqVar.c = a3.e;
                    xqVar.e = a3.d;
                    xqVar.g = Integer.valueOf(a3.f.longValue() != 0 ? 1 : 0);
                    if (this.e > 0) {
                        xqVar.d = adw.a(this.q) ? Long.valueOf(Math.round(((double) this.j) / ((double) this.e))) : null;
                        xqVar.f = Long.valueOf(Math.round(((double) this.i) / ((double) this.e)));
                    }
                    xqVar.j = a3.i;
                    xqVar.i = a3.j;
                    xqVar.k = Integer.valueOf(a3.k.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw e2) {
            }
            if (this.h > 0) {
                xqVar.l = Long.valueOf(this.h);
            }
            wrVar.K = xqVar;
            if (this.d > 0) {
                wrVar.y = Long.valueOf(this.d);
            }
            if (this.e > 0) {
                wrVar.x = Long.valueOf(this.e);
            }
            if (this.f > 0) {
                wrVar.w = Long.valueOf(this.f);
            }
            if (this.g > 0) {
                wrVar.z = Long.valueOf(this.g);
            }
            try {
                int size = this.c.size() - 1;
                if (size > 0) {
                    wrVar.L = new xq[size];
                    for (int i = 0; i < size; i++) {
                        a = a(a, (MotionEvent) this.c.get(i), this.q);
                        xq xqVar2 = new xq();
                        xqVar2.a = a.a;
                        xqVar2.b = a.b;
                        wrVar.L[i] = xqVar2;
                    }
                }
            } catch (zzcw e3) {
                wrVar.L = null;
            }
            ArrayList arrayList = new ArrayList();
            if (b.c() != null) {
                int n = b.n();
                arrayList.add(new aej(b, wrVar));
                arrayList.add(new aem(b, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", wrVar, n, 1));
                arrayList.add(new aeh(b, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", wrVar, v, n, 25));
                arrayList.add(new aeg(b, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", wrVar, n, 44));
                arrayList.add(new ael(b, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", wrVar, n, 12));
                arrayList.add(new aen(b, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", wrVar, n, 3));
                arrayList.add(new aek(b, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", wrVar, n, 22));
                arrayList.add(new aef(b, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", wrVar, n, 5));
                arrayList.add(new aev(b, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", wrVar, n, 48));
                arrayList.add(new aec(b, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", wrVar, n, 49));
                arrayList.add(new aes(b, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", wrVar, n, 51));
                arrayList.add(new aer(b, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", wrVar, n, 45, new Throwable().getStackTrace()));
                arrayList.add(new aew(b, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", wrVar, n, 57, view));
                arrayList.add(new aep(b, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", wrVar, n, 61));
                if (((Boolean) akc.f().a(amn.bE)).booleanValue()) {
                    arrayList.add(new aeb(b, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", wrVar, n, 62, view, activity));
                }
                if (((Boolean) akc.f().a(amn.bG)).booleanValue()) {
                    arrayList.add(new aet(b, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", wrVar, n, 53, this.z));
                }
            }
            Object asList2 = arrayList;
        } else {
            wrVar.k = Long.valueOf(16384);
            asList2 = Arrays.asList(new Callable[]{new aej(b, wrVar)});
        }
        a(asList2);
        return wrVar;
    }

    protected final wr a(Context context, tf tfVar) {
        wr wrVar = new wr();
        if (!TextUtils.isEmpty(this.w)) {
            wrVar.b = this.w;
        }
        adn b = b(context, this.r);
        if (b.c() != null) {
            a(a(b, wrVar, null));
        }
        return wrVar;
    }

    protected List<Callable<Void>> a(adn adn, wr wrVar, tf tfVar) {
        int n = adn.n();
        ArrayList arrayList = new ArrayList();
        if (adn.b()) {
            ArrayList arrayList2 = arrayList;
            arrayList2.add(new aee(adn, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", wrVar, n, 27, tfVar));
            arrayList2 = arrayList;
            arrayList2.add(new aeh(adn, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", wrVar, v, n, 25));
            arrayList2 = arrayList;
            arrayList2.add(new aem(adn, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", wrVar, n, 1));
            arrayList2 = arrayList;
            arrayList2.add(new aeo(adn, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", wrVar, n, 31));
            arrayList2 = arrayList;
            arrayList2.add(new aeu(adn, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", wrVar, n, 33));
            arrayList2 = arrayList;
            arrayList2.add(new aed(adn, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", wrVar, n, 29));
            arrayList2 = arrayList;
            arrayList2.add(new aef(adn, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", wrVar, n, 5));
            arrayList2 = arrayList;
            arrayList2.add(new ael(adn, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", wrVar, n, 12));
            arrayList2 = arrayList;
            arrayList2.add(new aen(adn, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", wrVar, n, 3));
            arrayList2 = arrayList;
            arrayList2.add(new aeg(adn, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", wrVar, n, 44));
            arrayList2 = arrayList;
            arrayList2.add(new aek(adn, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", wrVar, n, 22));
            arrayList2 = arrayList;
            arrayList2.add(new aev(adn, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", wrVar, n, 48));
            arrayList2 = arrayList;
            arrayList2.add(new aec(adn, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", wrVar, n, 49));
            arrayList2 = arrayList;
            arrayList2.add(new aes(adn, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", wrVar, n, 51));
            arrayList2 = arrayList;
            arrayList2.add(new aep(adn, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", wrVar, n, 61));
            return arrayList;
        }
        wrVar.k = Long.valueOf(16384);
        return arrayList;
    }

    public final void zzb(View view) {
        if (!((Boolean) akc.f().a(amn.bG)).booleanValue()) {
            return;
        }
        if (this.z == null) {
            this.z = new ady(a, view);
        } else {
            this.z.a(view);
        }
    }
}
