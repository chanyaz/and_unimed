...
public final class vm {
    ...
    public static KeyPair a(ECParameterSpec eCParameterSpec) {
        KeyPairGenerator keyPairGenerator = (KeyPairGenerator) vp.d.a("EC");
        keyPairGenerator.initialize(eCParameterSpec);
        return keyPairGenerator.generateKeyPair();
    }

    public static ECPublicKey a(zzayv zzayv, byte[] bArr, byte[] bArr2) {
        ECParameterSpec a = a(zzayv);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        a(eCPoint, a.getCurve());
        return (ECPublicKey) ((KeyFactory) vp.e.a("EC")).generatePublic(new ECPublicKeySpec(eCPoint, a));
    }

    public static ECParameterSpec a(zzayv zzayv) {
        switch (vn.b[zzayv.ordinal()]) {
            case 1:
    ...
    public static byte[] a(ECPrivateKey eCPrivateKey, ECPoint eCPoint) {
        ...
        KeyAgreement keyAgreement = (KeyAgreement) vp.c.a("ECDH");
        ...
        BigInteger mod = bigInteger.multiply(bigInteger).add(a).multiply(bigInteger).add(curve.getB()).mod(b);
        if (b.signum() != 1) {
        ...
        if (mod2.equals(BigInteger.ZERO)) {
            ...
            if (!b.testBit(0) || !b.testBit(1)) {
                if (b.testBit(0) && !b.testBit(1)) {
                    mod = BigInteger.ONE;
                    a = b.subtract(BigInteger.ONE).shiftRight(1);
                    bigInteger = mod;
                    int i = 0;
                    while (true) {
                        ...
                        BigInteger modPow = mod3.modPow(a, b);
                        if (modPow.add(BigInteger.ONE).equals(b)) {
                            BigInteger shiftRight = b.add(BigInteger.ONE).shiftRight(1);
                            a = BigInteger.ONE;
                            int bitLength = shiftRight.bitLength() - 2;
                            mod = bigInteger;
                            while (bitLength >= 0) {
                                BigInteger multiply = mod.multiply(a);
                                modPow = mod.multiply(mod).add(a.multiply(a).mod(b).multiply(mod3)).mod(b);
                                mod = multiply.add(multiply).mod(b);
                                if (shiftRight.testBit(bitLength)) {
                                    a = modPow.multiply(bigInteger).add(mod.multiply(mod3)).mod(b);
                                    mod = bigInteger.multiply(mod).add(modPow).mod(b);
                                } else {
                                    a = modPow;
                                }
                                bitLength--;
